...
- **`max-width` now constrains a block's content width** — text wraps inside the `max-width` rather than the full container width. Absolute lengths only; `%` remains unsupported. (`min-width` is still parsed-but-not-applied.)
- **Docs correction**: `CSS_PROPERTIES_MATRIX.md` previously over-claimed several properties as supported (`text-align`/`text-indent` didn't execute, `min-width`/`border-spacing` weren't implemented). The matrix now reflects what actually executes.
