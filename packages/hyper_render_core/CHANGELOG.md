...
- **`max-width`**: executed through the line-breaker's existing per-block padding stack — a `max-width` block inflates its right inset so text wraps within the constraint. Also emits a block-start fragment for a first block that has no top margin (previously elided), which is what carries the constraint. Absolute lengths only.
