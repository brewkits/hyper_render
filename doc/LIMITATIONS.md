...
| `float` on a text-sized block | A float reserves a box from the element's `width`/`height`; it does not lay the floated element's own content out inside that box. So `<img style="float:left">` and `<div style="float:left;width:150px;height:120px">` both wrap text correctly, but `<div style="float:left;width:150px">…text…</div>` is **not** floated — its text renders in normal flow and the following paragraph does not wrap beside it. Pinned by `test/flagship_execution_audit_test.dart` |
| `min-height` / `max-height` | Parsed into `ComputedStyle`, then never read by the render path — they have no effect on any element. Previously marked fully supported in the CSS matrix; corrected 2026-08-16 after a differential render test measured no change |
| `height` on a non-replaced block | `height` is applied to replaced elements (`<img>`) and to float boxes only. `<div style="height:200px">` renders at its content height |
