...
- [x] `animation-play-state: running/paused` — parsed + executed at both tiers; a paused animation holds its current frame and resumes from it (unreleased — see CHANGELOG `[Unreleased]`)
- [x] Block-level `animation-name` execution on the **canvas paint tier** (`RenderHyperBox`) — previously only widget-tier content (flex/grid containers, plugins, atomic elements) animated; plain paragraphs/divs painted directly on the canvas were inert. Frame-driven via `SchedulerBinding.scheduleFrameCallback` (same no-`TickerProvider` pattern as the image-loading shimmer, not an `AnimationController`). Opacity is composited via a single `saveLayer` per animated block (decoration + text painted together, then faded once) rather than per-fragment, to avoid double-blending a block's own background showing through its own text. V1 scope: a block's own decoration + its directly-owned text/ruby fragments only — list markers, inline `<span>` decorations, floated images, and animations on a *nested* animated descendant do not compose with an animating ancestor's transform/opacity (unreleased — see CHANGELOG `[Unreleased]`)
